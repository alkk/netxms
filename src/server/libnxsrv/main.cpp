 ...
 * Global variables
 */
LIBNXSRV_EXPORTABLE_VAR(UINT64 g_flags) = AF_USE_SYSLOG | AF_CATCH_EXCEPTIONS | AF_WRITE_FULL_DUMP | AF_ENABLE_LOCAL_CONSOLE | AF_CACHE_DB_ON_STARTUP;
...
{
   return -(COMPARE_NUMBERS(static_cast<const ROUTE*>(p1)->dwDestMask, static_cast<const ROUTE*>(p2)->dwDestMask));
}
...
{
   routingTable->sort(CompareRoutes);
}
